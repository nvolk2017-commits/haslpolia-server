...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    // ── GAME ACTION (host sends updated state to all) ──
      ...
      // initRoll, auctionBid, auctionStart, auctionEnd рассылаем ВСЕМ включая отправителя
      const broadcastToAll = ['initRoll', 'auctionBid', 'auctionStart', 'auctionEnd', 'showCard', 'log', 'captureOpen', 'captureAtk', 'captureDef', 'captureEnd'];
      if (broadcastToAll.includes(data.action)) {
        broadcastAll(ws.roomCode, {
          type: 'stateUpdate',
          data: { state: data.state, action: data.action, payload: data.payload }
        });
      } else {
